...
app.post("/buscarArchivo", async (req, res) => {
  ...
  conn.query("SELECT Nombre, Ruta,  (SELECT username FROM Usuario WHERE idUsuario=Archivo.Usuario_idUsuario)  as propietario, Archivo.Usuario_idUsuario as idUsuario FROM Archivo WHERE Archivo.Nombre LIKE ? AND Archivo.Visibilidad = 1 AND (  Archivo.Usuario_idUsuario IN (SELECT Usuario_idUsuario FROM Amigos WHERE USuario_idUsuario1= ? )  OR Archivo.Usuario_idUsuario IN (SELECT Usuario_idUsuario1 FROM Amigos WHERE Amigos.Usuario_idUsuario = ? ));",[nombre,body.idUsuario,body.idUsuario], function (err, result) {
    if (err) throw err;
    console.log(result);
    res.send(result);
  });
